...
def interp_monthly_coeff_daily(monthly_coeffs):
    '''  Convert monthly coefficients to daily values '''
...
def eq_temp(rtw,at,cl,ws,sr,td,eq_temp_out):
    ''' EQUILIBRIUM TEMPERATURE COMPUTATION DRIVER'''
...
def invent_elevation(res_name,forecast_dss,storage_rec,elev_constant_ft):
    ''' CREATE CONSTANT ELEVATION TIME SERIES'''
...
def study_dir_from_run_dir(run_dir):
    ''' Extract study directory by stripping run and model folders'''
...
def model_dir_from_run_dir(run_dir,model_place,model_name):
    ''' Construct full model path from study directory structure'''
...
def get_downstream_loc(forecastDSS):
    ''' Read downstream control location identifier from DSS'''
...
def snap_inital_gate_elev_from_folsom_elev(elev_in):
    '''
    Mapping function that converts ResSim "true shutter" elevations to the
    corresponding CE-QUAL-W2 starting gate elevations.
    
    The W2 model does not use the same elevation scale as ResSim, so a set of
    discrete elevation ranges are translated into predefined W2 gate positions.
...
def get_initial_shutter_positions(rtw, currentAlternative, computeOptions, shutter_objs):
    '''
    Reads initial forecast gate positions from DSS, converts them to the exact
    CE-QUAL-W2 starting shutter elevations, and returns the values for writing
    into the W2 control file.

    w2_con.csv:
        Rows 166, 167, and 168 define the starting gate conditions for the
        three shutters.
        ...
        - Top shutter removed:
              [113.05, 124.94, 124.94]

        - Simulations beginning before the temperature management season
          (DOY <= 120) use penstock defaults:
              [93.57, 93.57, 93.57]
    '''
